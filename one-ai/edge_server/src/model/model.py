...
class DanceDecoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # if rotary, replace absolute embedding with a rotary embedding instance
        # (absolute becomes an identity)
        if use_rotary:
...
class DanceDecoderForSNPE(nn.Module):
    def __init__(
        self,
        nfeats: int,
        seq_len: int = 150,  # 5 seconds, 30 fps
    ...
    def forward(
        ...
        null_cond_embed = self.null_cond_embed.to(cond_tokens.dtype)
        if cond_drop_prob == 1:
            cond_tokens = null_cond_embed.repeat((cond_tokens.shape[0], 1, 1))
        ...
        # cond_hidden = torch.where(keep_mask_hidden, cond_hidden, null_cond_hidden)
        if cond_drop_prob == 1:
            cond_hidden = null_cond_hidden.repeat((cond_hidden.shape[0], 1))
        t += cond_hidden

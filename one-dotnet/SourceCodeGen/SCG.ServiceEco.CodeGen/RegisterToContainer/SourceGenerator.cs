...
namespace TPFive.SCG.ServiceEco.CodeGen.RegisterToContainer
{
    ...
    public class SourceGenerator : ISourceGenerator
    {
            ...
            Compilation compilation)
        {
            ...
            var scope = GetScopeValue(classSymbol);
            ...
            return ($"{classModel.Name}.Installer.Generated.cs", source);
        }

        private static int GetScopeValue(ISymbol classSymbol)
        {
            var specificAttribute = classSymbol
                .GetAttributes()
                .FirstOrDefault(c => c.AttributeClass.Name == nameof(RegisterToContainerAttribute));
            var scope = specificAttribute.GetAttributeValue(
                nameof(RegisterToContainerAttribute.Scope),
                1);

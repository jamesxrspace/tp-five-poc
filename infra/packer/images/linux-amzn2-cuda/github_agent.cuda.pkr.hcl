...
locals {
  runner_version = coalesce(trimprefix(jsondecode(data.http.github_runner_release_json.body).tag_name, "v"), var.runner_version)
}

source "amazon-ebs" "githubrunner" {
  ami_name                                  = "github-runner-cuda-x86_64-${formatdate("YYYYMMDDhhmm", timestamp())}"
  ...
  source_ami_filter {
    filters = {
      name                = "Deep Learning AMI GPU PyTorch 1.13.1 (Amazon Linux 2) 20230818"
...
build {
  ...
  provisioner "shell" {
    ...
    inline = concat([
      "sudo adduser runner",
      "sudo yum update -y",
      "sudo yum install -y amazon-cloudwatch-agent curl jq git wget",
      "sudo amazon-linux-extras install docker epel",
      "sudo yum update -y",
